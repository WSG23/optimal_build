    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <aside className="lg:col-span-1 border border-border-neutral/40 rounded-lg p-3 bg-surface-inverse/40">
          <h3 className="text-sm font-semibold text-text-inverse mb-2">
            Rules
          </h3>
          <ul className="space-y-1">
            ...
            {diffs.length === 0 && (
              <li className="text-sm text-text-inverse/70">
                No diffs available.
              </li>
            )}
          </ul>
        ...
        <section className="lg:col-span-3 border border-border-neutral/40 rounded-lg p-4 bg-surface-inverse/40">
          {activeDiff ? (
            <div>
              <h3 className="text-sm font-semibold text-text-inverse mb-4">
                Clause Change
              </h3>
              <DiffViewer
                baseline={activeDiff.baseline}
                updated={activeDiff.updated}
              />
            </div>
          ) : (
            <p className="text-sm text-text-inverse/70">
              Select a rule to inspect its diff.
            </p>
          )}
        </section>
      </div>
