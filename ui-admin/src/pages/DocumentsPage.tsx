...
const DocumentsPage = () => {
  ...
  return (
    <div>
      ...
      />
      {loading && <p className="text-sm text-text-inverse/70">Loading documents…</p>}
      {error && <p className="text-sm text-error-strong/85">{error}</p>}
      {!loading && !error && (
        <ul className="space-y-3">
          {documents.map((document) => (
            <li key={document.id} className="border border-border-neutral/40 rounded-lg p-4 bg-surface-inverse/40">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-text-inverse">Version {document.version_label || 'N/A'}</p>
                  <p className="text-xs text-text-inverse-muted/90">Storage: {document.storage_path}</p>
                </div>
