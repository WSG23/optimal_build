...
async def test_edgeprop_client_end_to_end():
    ...
    listing_id, echoed = await client.publish_listing(
        {"external_id": "edge-1", "price": 100}
    )
...
async def test_propertyguru_client_end_to_end():
    ...
    listing_id, echoed = await client.publish_listing(
        {"external_id": "guru-1", "title": "Sample"}
    )
...
async def test_zoho_client_end_to_end():
    ...
    lead_id, echoed = await client.publish_listing(
        {"external_id": "lead-1", "name": "Lead"}
    )
