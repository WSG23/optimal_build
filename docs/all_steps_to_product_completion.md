...
- **2026-04-03:** Epic 1 foundation tranche (Codex) — Added machine-readable external source metadata via [`backend/app/schemas/external_sources.py`](../backend/app/schemas/external_sources.py) and surfaced live/mock/unavailable state on geocoding responses, listing integration responses, and CORENET-backed regulatory submissions. Extended that provenance contract into the main GPS capture workflows for both agent and developer experiences, including OneMap amenity status, so SG-facing capture results now expose external data state instead of silently dropping it. This creates an explicit contract for Singapore-first integration hardening and removes silent ambiguity around synthetic provider behavior. Verified with `.venv/bin/pytest backend/tests/services/test_geocoding_service.py backend/tests/test_api/test_listing_integrations.py backend/tests/test_services/test_mock_corenet.py backend/tests/unit/services/test_regulatory_service.py backend/tests/test_api/test_agents_gps.py backend/tests/test_api/test_developer_site_acquisition.py backend/tests/test_integration/test_preview_job_integration.py unit_tests/test_gps_property_logger.py` (68 passed across the two runs) and `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m py_compile ...` on touched backend modules.
- **2026-04-03:** Singapore-first product surface hardening (Codex) — Refocused the primary frontend UX around Singapore developers by turning [`frontend/src/app/pages/projects/ProjectListPage.tsx`](../frontend/src/app/pages/projects/ProjectListPage.tsx) into a real onboarding hub, hiding non-SG jurisdiction choices from the visible capture/advisory flows, replacing mocked listing connects in [`frontend/src/app/pages/integrations/IntegrationsPage.tsx`](../frontend/src/app/pages/integrations/IntegrationsPage.tsx) with explicit partner-gated states, adding SG finance templates in [`frontend/src/modules/finance/components/FinanceScenarioCreator.tsx`](../frontend/src/modules/finance/components/FinanceScenarioCreator.tsx), exposing dedicated evidence packs in [`frontend/src/app/pages/evidence/EvidencePage.tsx`](../frontend/src/app/pages/evidence/EvidencePage.tsx), and publishing a public-facing Excel comparison page in [`frontend/src/app/pages/why-not-excel/WhyNotExcelPage.tsx`](../frontend/src/app/pages/why-not-excel/WhyNotExcelPage.tsx). Added a finance release gate to [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) plus origin/regression coverage in [`backend/tests/test_api/test_finance_origin_classification.py`](../backend/tests/test_api/test_finance_origin_classification.py). Verified with `./frontend/node_modules/.bin/tsc -p frontend/tsconfig.json --noEmit`, `./frontend/node_modules/.bin/vitest run src/app/pages/projects/ProjectListPage.test.tsx src/app/pages/integrations/IntegrationsPage.test.tsx src/api/__tests__/finance.test.ts src/modules/finance/__tests__/quickScreenDraft.test.ts`, and `.venv/bin/pytest backend/tests/test_api/test_finance_origin_classification.py backend/tests/test_api/test_finance_workbook.py backend/tests/test_api/test_audit.py backend/tests/test_core/test_audit_ledger.py backend/tests/test_services/test_finance_golden_deals.py -q`.
- **2026-04-03:** Epic 2 compliance hardening tranche (Codex) — Upgraded Singapore URA/BCA compliance outputs in [`backend/app/utils/singapore_compliance.py`](../backend/app/utils/singapore_compliance.py) so every check now reports applied rule evidence, corpus coverage state, review-status counts, and confidence instead of only pass/fail summaries. The proposed-design compliance API in [`backend/app/api/v1/singapore_property_api.py`](../backend/app/api/v1/singapore_property_api.py) now surfaces those fields directly, making it explicit when a zoning/building result is backed by approved published rules versus pending or missing corpus coverage. Verified with `.venv/bin/pytest backend/tests/utils/test_singapore_compliance.py backend/tests/test_utils/test_singapore_compliance.py backend/tests/test_api/test_singapore_property_api.py` (39 passed) and `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m py_compile backend/app/utils/singapore_compliance.py backend/app/api/v1/singapore_property_api.py`.
- **2026-04-03:** Epic 1/2 buildable visibility pass (Codex) — Extended SG rule-corpus visibility into indirect buildability consumers: [`backend/app/services/buildable.py`](../backend/app/services/buildable.py), [`backend/app/api/v1/screen.py`](../backend/app/api/v1/screen.py), [`backend/app/api/v1/singapore_property_api.py`](../backend/app/api/v1/singapore_property_api.py), and the developer capture envelope in [`backend/app/api/v1/developers_gps.py`](../backend/app/api/v1/developers_gps.py). Buildable and developer-envelope responses now expose corpus coverage state, confidence, review counts, and applied rule IDs instead of hiding whether SG rule inputs are approved, partial, pending review, or mocked. Verified with `.venv/bin/pytest backend/tests/test_services/test_buildable.py backend/tests/test_api/test_screen_api.py backend/tests/pwp/test_buildable_golden.py backend/tests/pwp/test_buildable_metrics.py backend/tests/pwp/test_buildable_request_aliases.py backend/tests/test_api/test_singapore_property_api.py backend/tests/test_api/test_developer_site_acquisition.py` (31 passed) and `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m py_compile backend/app/schemas/buildable.py backend/app/services/buildable.py backend/app/services/rules/zone_rules.py backend/app/api/v1/screen.py backend/app/api/v1/singapore_property_api.py backend/app/api/v1/developers_common.py backend/app/api/v1/developers_gps.py`.
- **2026-04-03:** Epic 2 governance pass (Codex) — Added corpus-governance visibility to the admin review surface in [`backend/app/api/v1/review.py`](../backend/app/api/v1/review.py) via `/review/corpus-status`, summarising SG source freshness, suspected upstream updates, review queue size, approved/published/traceable rule counts, and overall corpus coverage state by authority/topic source. This closes the remaining product-facing gap in Epic 2: compliance now has both runtime evidence on user-facing checks and an explicit admin endpoint for managing the BCA/URA corpus lifecycle. Verified with `.venv/bin/pytest backend/tests/test_api/test_review_api.py backend/tests/utils/test_singapore_compliance.py backend/tests/test_utils/test_singapore_compliance.py backend/tests/test_api/test_singapore_property_api.py` (44 passed) and `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m py_compile backend/app/api/v1/review.py`.
- **2026-04-03:** Epic 1/2 feasibility provenance pass (Codex) — Extended SG rule-corpus provenance into the generic feasibility layer by teaching [`backend/app/schemas/feasibility.py`](../backend/app/schemas/feasibility.py) and [`backend/app/services/feasibility/__init__.py`](../backend/app/services/feasibility/__init__.py) to carry build-envelope rule corpus status through rule selection and feasibility assessment responses. The feasibility APIs now preserve SG corpus quality even when zoning rules are consumed indirectly via the generic engine, and the frontend geocoding client in [`frontend/src/api/geocoding.ts`](../frontend/src/api/geocoding.ts) now retains machine-readable source metadata instead of discarding it. Verified with `.venv/bin/pytest backend/tests/test_api/test_feasibility.py backend/tests/test_services/test_feasibility.py backend/tests/test_services/test_feasibility_engine.py` (17 passed at the time of the tranche) and `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m py_compile backend/app/schemas/feasibility.py backend/app/services/feasibility/__init__.py`.
- **2026-04-03:** Epic 3 standalone calculator tranche (Codex) — Added a dedicated Singapore deal calculator backend in [`backend/app/api/v1/developers_deal_calculator.py`](../backend/app/api/v1/developers_deal_calculator.py) with request/response contracts in [`backend/app/schemas/deal_calculator.py`](../backend/app/schemas/deal_calculator.py), then surfaced it in the developer router via [`backend/app/api/v1/developers.py`](../backend/app/api/v1/developers.py). The new flow accepts either an address or manual parameters, composes geocoding, URA metadata, buildable screening, feasibility, and lightweight finance metrics into one response, and exposes provenance and rule-corpus quality alongside IRR/NPV/DSCR/MOIC-style top-of-funnel outputs. On the frontend, added a dedicated client in [`frontend/src/api/dealCalculator.ts`](../frontend/src/api/dealCalculator.ts), provenance helpers in [`frontend/src/api/externalSources.ts`](../frontend/src/api/externalSources.ts), a new page in [`frontend/src/app/pages/deal-calculator/DealCalculatorPage.tsx`](../frontend/src/app/pages/deal-calculator/DealCalculatorPage.tsx), and routes/navigation in [`frontend/src/main.tsx`](../frontend/src/main.tsx) and [`frontend/src/app/navigation.ts`](../frontend/src/app/navigation.ts). Verified with `.venv/bin/pytest backend/tests/test_api/test_developers_deal_calculator.py backend/tests/test_api/test_feasibility.py backend/tests/test_services/test_feasibility.py backend/tests/test_services/test_feasibility_engine.py` (19 passed), `npx vitest run frontend/src/api/__tests__/dealCalculator.test.ts` (1 passed), and `PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m py_compile backend/app/schemas/feasibility.py backend/app/services/feasibility/__init__.py backend/app/schemas/deal_calculator.py backend/app/api/v1/developers_deal_calculator.py backend/app/api/v1/developers.py`.
...
- **2 vendored shim dirs at repo root** (`geoalchemy2/`, `prefect/` + `backend/prefect/`). The first round (`ede4b48f`) removed all six; PR #320 CI exposed two that aren't merely "package not installed" fallbacks and were rolled back (`db63b54b`, `dbf81e55`): real geoalchemy2 emits `RecoverGeometryColumn` calls on SQLite that the `backend/aiosqlite/` shim can't service, and `tests/test_flows/test_watch_fetch_flow.py` asserts the shim's identity-preserving `with_options`. Removing these now means either adding the spatialite functions to the aiosqlite shim, or rewriting the prefect identity tests to assert real-prefect semantics. The vendored pydantic stub note is obsolete — `model_validate` now correctly returns `Self`, and the `mypy-app.schemas._typing` allowlist entry has been dropped.
...
### Codex Progress Notes

- Epic 4 workbook tranche shipped: new backend Excel export/import preview/import APIs, workbook exchange service, and finance workspace workbook UI.
- Epic 3 follow-up shipped: standalone deal calculator now preserves asset breakdowns on the frontend and can hand off a seeded quick-screen finance draft into the finance workspace.
- Epic 5 shipped as a golden validation pack: locked finance regression coverage now asserts NPV, IRR, DSCR bucket behavior, MOIC/equity multiple analytics, construction-loan interest schedules, and sensitivity outcomes against deterministic SG-style fixtures.
- Epic 6 shipped as a workspace split pass: agent and developer routes now mount into workspace-specific shells/sidebar trees, with dedicated `/agents` and `/developers` dashboard entry points and the shared capture/finance surfaces mounted under the correct workspace wrapper.
- Epic 7 shipped as an audit evidence surface: `/api/v1/audit/{project_id}/evidence` now produces an evidence-pack summary including chain integrity, event counts, export artifacts, recipients, scenario events, and submission events.
- Epic 8 shipped as a CORENET adapter pass: regulatory submissions now run through an explicit CORENET adapter boundary, advertise capability via `/api/v1/regulatory/corenet-capability`, and default to submission-prep semantics with live-submit gated off unless a real adapter is configured.
- Full-pass hardening complete: the deal calculator now surfaces rule-corpus/source-quality signals in the UI, and the regulatory dashboard now reflects real submission-prep capability state instead of static “online” messaging while rendering human-readable agency labels instead of raw UUIDs.
- Follow-up hardening pass complete: audit evidence now records workbook import artifacts separately from exports, finance workbook import/export and regulatory submission flows append first-class audit ledger events, and evidence summaries now preserve agency names plus workbook metadata instead of collapsing those actions into generic format-based buckets.
- Follow-up handoff pass complete: quick-screen finance drafts now preserve rule-corpus status, source caveats, and recommended rule IDs across the deal-calculator-to-finance transition, the finance workspace shows that trust context before import, and imported finance scenarios retain the quick-screen assessment summary in their persisted description.
- Follow-up finance audit pass complete: the shared finance feasibility pipeline now appends `finance_scenario_created` ledger events so direct finance runs, quick-screen imports, and workbook-driven scenario creation all enter the Epic 7 evidence trail instead of only workbook export/import actions being auditable.
- Follow-up finance provenance pass complete: workbook imports now stamp `[Workbook import context]` before entering the shared finance feasibility pipeline, so `finance_scenario_created` audit events classify imported scenarios as `workbook` instead of collapsing them into generic manual finance runs. Quick-screen, workbook, and manual finance origins are now distinct in the audit trail.
- Follow-up evidence-report pass complete: the audit evidence summary now exposes finance scenario origin metadata directly, including a dedicated `finance_events` section plus `origin` on generic scenario events, so manual, quick-screen, and workbook-created scenarios are distinguishable without inspecting raw ledger context.
- Follow-up audit-consumption pass complete: added `/api/v1/audit/by-ref/{project_ref}/evidence` so UUID/string project references can resolve into audit evidence without leaking integer chain IDs into the frontend, then surfaced a compact audit evidence snapshot inside the finance workspace with finance-origin, workbook-import, and submission-prep highlights.
