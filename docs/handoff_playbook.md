...
- Points to the authoritative sources so status stays consistent with `docs/ROADMAP.MD` and `docs/WORK_QUEUE.MD`.
...
3. `README.md` – onboarding note that requires `ROADMAP.MD`, `WORK_QUEUE.MD`, and `NEXT_STEPS_FOR_AI_AGENTS_AND_DEVELOPERS.md`.
4. `docs/ROADMAP.MD` – project status, acceptance criteria, progress ledger (paired with `docs/WORK_QUEUE.MD` for task detail).
...
- Run and complete the agent live walkthroughs, capture findings in `docs/WORK_QUEUE.MD` (Active/Ready updates) and `docs/ROADMAP.MD`, and file follow-up tickets with `agent-validation` label.
- Keep `docs/ROADMAP.MD` as the live status log and `docs/WORK_QUEUE.MD` as the execution log—update both with every change.
...
- Update `docs/WORK_QUEUE.MD` immediately—move tasks, record blockers, and add completion context—then refresh `docs/ROADMAP.MD` snapshot/dependencies.
