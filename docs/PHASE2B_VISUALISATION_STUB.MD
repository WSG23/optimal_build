# Phase 2B Visualisation Delivery Plan (v0.5)
...
- **v0.5 (2025-11-04)** – GLTF renderer + preview viewer delivered.
  - Replaced the JSON-only stub with a versioned GLTF + BIN pipeline, including thumbnail generation and an asset manifest for downstream services.
  - Preview jobs now persist `metadata_url`/`asset_version`, and the Site Acquisition UI renders the GLTF via a Three.js viewer with camera-orbit hints.
  - Documented outstanding follow-ups (manual QA execution, monitoring dashboards) tied to the new workflow.
...
- MVP shipped as a Python GLTF builder inside `backend/app/services/preview_generator.py`, using struct-packed geometry and Pillow thumbnails (no external binary dependency).
- Inputs: geometry payload JSON, colour palette, and camera hints; outputs `preview.gltf`, accompanying `preview.bin`, `preview.json` (metadata + manifest), and `thumbnail.png`.
- Preview jobs update `metadata_url`, `asset_version`, and manifest fields when the job reaches `READY`; reruns clear the manifest before requeueing.
- Stretch goal: revisit a photorealistic renderer (Blender/Three.js service) if stakeholders require richer visuals in Phase 2C.


#### 4.4.1 Level 2 Detail Enhancement (Phase 2B Residual)

**Current State (Level 1):** Simple box geometry with square footprints, basic height extrusion, generation time <1 second.

**Target State (Level 2 - Recommended):** Medium-detail architectural massing suitable for early-stage developer presentations and feasibility review.

**Level 2 Features:**

1. **Octagonal Footprints**
   - Replace square footprints with 8-sided polygons to approximate complex building shapes
   - Vertices at 45° angles for chamfered corners
   - Maintains performance while adding architectural character

2. **Basic Setbacks for Tall Buildings**
   - Apply two setback tiers for buildings >60m height:
     - First setback at 1/3 total height (reduces footprint by 15%)
     - Second setback at 2/3 total height (reduces footprint by additional 10%)
   - Creates tapered massing typical of modern high-rises

3. **Ground Floor Podium Distinction**
   - Separate ground floor geometry (typically 4.5-6m height)
   - Visual distinction for retail/commercial base in mixed-use developments
   - Different material/color in GLTF for podium vs tower

4. **Floor Lines Every 3.5m**
   - Add horizontal edge lines at typical floor intervals (3.5m spacing)
   - Provides scale reference and architectural detail
   - Implemented as additional geometry primitives (thin quads)

5. **Basic Ambient Shadows**
   - Simple ambient occlusion approximation at ground level
   - Darker vertex colors at base, gradient to lighter at top
   - No raytracing required - pre-computed vertex colors

6. **Thumbnail Optimization**
   - 45° isometric view (not straight-on orthographic)
   - Camera positioned at 30-45° elevation for better depth perception
   - 512x512 PNG thumbnail with transparent background

**Performance Target:**
- Generation time: <5 seconds per preview (acceptable for background job queue)
- Geometry complexity: 100-500 vertices per building (up from current 8-24)
- File size: <200KB for GLTF+BIN combined

**Implementation Guidance:**
- Modify `preview_generator.py::_serialise_layer()` to support geometry mode parameter:
  - `mode="simple"` (current Level 1 - square footprint)
  - `mode="medium"` (Level 2 - octagonal + setbacks + floor lines)
  - `mode="detailed"` (future Level 3 - facade articulation)
- Add `geometry_detail_level` setting to `PreviewJob` model (default "medium")
- Camera position configured via `preview_generator.py::_generate_thumbnail()` camera hints

**Level 3 (Future Enhancement - Not Phase 2B):**
- Facade articulation (window grids, balconies)
- Material textures (glass, concrete, brick)
- Rooftop features (mechanical, amenities)
...
- Expire assets older than N versions (configurable, default keep last 3) and mark jobs as `expired` when pruned. Use `backend/scripts/preview_cleanup.py --max-versions <N>` (wired to `settings.PREVIEW_MAX_VERSIONS`) to prune old directories and update `PreviewJob` rows.
- Metadata now records an `asset_manifest` block (GLTF, BIN, metadata, thumbnail, version) for downstream services.
...
- **Metrics** (Prometheus):
  - `preview_generation_jobs_total{scenario,backend}` – enqueued jobs.
  - `preview_generation_jobs_completed_total{outcome}` – completions grouped by outcome.
  - `preview_generation_job_duration_ms{outcome}` – end-to-end latency histogram.
  - `preview_generation_queue_depth{backend}` – queued jobs awaiting workers.
...
1. Renderer technology choice (Blender vs. custom Three.js vs. existing CAD service). **Status:** Resolved for MVP (custom Python GLTF builder); reassess for photorealistic stretch. **Owner:** Platform + Visualisation.
2. Where to source accurate footprints (do we derive from URA site boundary or external CAD uploads?). **Owner:** Geometry.
3. **Asset retention policy:** Keep the most recent five versions per property (or any asset older than 30 days) and mark jobs whose assets are pruned as `expired`. Implemented via a nightly housekeeping task that removes files from `static/` or S3 and updates the job record. **Owner:** Platform.
4. **Authentication for asset URLs:** Serve preview assets from S3 with short-lived (15 min) pre-signed URLs issued by the backend; fall back to a FastAPI streaming proxy for environments without S3. Access is therefore tied to the developer session instead of public buckets. **Owner:** Security.
5. **Per-scenario renders:** Persist one GLTF bundle per scenario (`base`, `expansion`, `reposition`, etc.) under `static/dev-previews/{property_id}/{version}/{scenario}/`. The API exposes the scenario map so the viewer can switch between meshes without re-enqueuing the job. **Owner:** Product.

---

## 9. Next Steps Checklist

- [x] Finalise renderer selection + infrastructure requirements.
...
- [x] Define monitoring dashboards and alerts _(Prometheus metrics emitted; Grafana dashboard JSON at `infra/observability/grafana/phase2b_preview_dashboard.json` and Prometheus alert rules at `infra/observability/prometheus/preview_generation.rules.yml`)_.
...
- [x] Prefect worker (or inline CLI) seeded with a property that has optimiser outputs: `python -m scripts.preview enqueue --property-id <TEST_PROPERTY_ID>`
- [x] Frontend dev server: `cd frontend && npm run dev`
- [x] Browser at `http://localhost:3000/agents/developers/<TEST_PROPERTY_ID>/preview`

_Execution note (2025-11-04):_ Backend/API confirmed healthy via CLI POST to `/api/v1/developers/properties/log-gps` (stubbed renderer) returning `status=ready`; standalone preview route renders the massing harness in Chrome locally.
...
1. **Trigger render:** From the Site Acquisition view, click **“Generate Preview”** (or run the CLI command) and confirm a job id appears in the developer console/logs.
   - [x] Status banner transitions from `placeholder` → `processing` within the UI. _(Observed via CLI trigger + inline queue; UI briefly showed “Processing” before ready.)_
2. **Poll for completion:** Leave the page open until the status flips to `ready` (or `failed`).
   - [x] Confirm the thumbnail placeholder updates to the new asset timestamp. _(Timestamp updated to `2025-11-02 13:22` for seeded job.)_
   - [ ] If `failed`, capture the error message surfaced in the UI. _(Not exercised; no failures encountered.)_
3. **Inspect payload:** Use the UI “View JSON” affordance (or network tab) to download the generated payload.
   - [x] Bounding box, layer counts, and camera hints match expectations from backend logs. _(Verified against `preview.json` under `static/dev-previews/6a0b64be-fc56-458b-87fc-7c9b17c24bff/20251102132200-24a62073/`.)_
4. **Visual verification:** Rotate the preview in the test harness viewer.
   - [x] Massing layers render with colours and heights matching the optimiser seed data. _(Viewer shows 60 % office / 40 % retail stacks; noted that geometry is still MVP placeholder.)_
5. **Regression capture:** Take a screenshot of the loaded preview (attach to PR if possible) and record any anomalies in `docs/development/testing/known-issues.md` if they are harness-specific.
   - [x] Screenshot captured & linked. _(2025-11-04 manual QA screenshots attached in project log; geometry fidelity flagged for follow-on renderer milestone.)_

### Exit Criteria

- [x] Checklist completed with pass/fail notes.
- [x] Screenshot and/or console logs attached to PR/work queue note.
- [x] Any failures triaged (fixed immediately or documented with owner + follow-up task). _(No failures observed during run.)_

_Async verification note (2025-11-10):_ The same checklist was re-run with `JOB_QUEUE_BACKEND=rq`, Redis, and an RQ worker (`.venv/bin/rq worker preview`). With `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES` and `API_RATE_LIMIT=120/minute`, the Site Acquisition UI clearly stepped through `QUEUED → PROCESSING → READY`, confirming the production-style queue behaves as expected.
