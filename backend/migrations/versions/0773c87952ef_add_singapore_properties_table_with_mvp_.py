...
def upgrade() -> None:
    ...
    op.create_table('singapore_properties',
        ...
        # Property Details
        sa.Column('zoning', sa.Enum('residential', 'commercial', 'industrial', 'mixed_use',
                                   'business_park', 'civic_institutional', 'educational',
                                   'healthcare', 'transport', 'open_space', 'special_use',
                                   name='propertyzoning'), nullable=True),
        sa.Column('tenure', sa.Enum('freehold', '999_year_leasehold', '99_year_leasehold',
                                   '60_year_leasehold', '30_year_leasehold',
                                   name='propertytenure'), nullable=True),
        ...
        # Status
        sa.Column('development_status', sa.Enum('vacant_land', 'planning', 'approved',
                                               'under_construction', 'top_obtained',
                                               'csc_obtained', 'operational',
                                               name='developmentstatus'),
                 server_default='vacant_land', nullable=True),
        ...
        sa.Column('acquisition_status', sa.Enum('available', 'under_review', 'acquired', 'rejected',
                                               name='acquisitionstatus'),
                 server_default='available', nullable=True),
        sa.Column('feasibility_status', sa.Enum('analyzing', 'approved', 'rejected', 'on_hold',
                                               name='feasibilitystatus'),
                 server_default='analyzing', nullable=True),
        ...
        sa.Column('bca_compliance_status', sa.Enum('pending', 'passed', 'warning', 'failed',
                                                  name='compliancestatus'),
                 server_default='pending', nullable=True),
        sa.Column('ura_compliance_status', sa.Enum('pending', 'passed', 'warning', 'failed',
                                                  name='compliancestatus'),
                 server_default='pending', nullable=True),
        sa.Column('compliance_notes', sa.Text(), nullable=True),
        sa.Column('compliance_data', postgresql.JSON(astext_type=sa.Text()), nullable=True),
        ...
        sa.Column('ura_guidelines', postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column('nearby_amenities', postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column('development_constraints', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    ...
    op.create_index('ix_singapore_properties_owner_email', 'singapore_properties', ['owner_email'])
    op.create_index('ix_singapore_properties_project_id', 'singapore_properties', ['project_id'])
    op.create_index('ix_singapore_properties_postal_code', 'singapore_properties', ['postal_code'])
    op.create_index('ix_singapore_properties_acquisition_status', 'singapore_properties', ['acquisition_status'])
