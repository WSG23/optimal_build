...
def upgrade() -> None:
    ...
    op.create_table('properties',
        ...
        sa.Column('is_conservation', sa.Boolean(), server_default='false', nullable=True),
    ...
    op.create_index('idx_property_location', 'properties', ['location'], postgresql_using='gist')
    ...
    op.create_index('idx_transaction_property_date', 'market_transactions', ['property_id', 'transaction_date'])
    ...
    op.create_index('idx_rental_property_active', 'rental_listings', ['property_id', 'is_active'])
    ...
    op.create_table('development_pipeline',
        ...
        sa.Column('developer', sa.String(255), nullable=True),
        sa.Column('project_type', sa.Enum('office', 'retail', 'industrial', 'residential', 'mixed_use', 
                                         'hotel', 'warehouse', 'land', 'special_purpose', 
                                         name='property_type', create_type=False), nullable=False),
        sa.Column('location', Geography(geometry_type='POINT', srid=4326), nullable=False),
        ...
        sa.Column('development_status', sa.Enum('existing', 'planned', 'approved', 'under_construction', 
                                               'completed', 'demolished', name='property_status', create_type=False), 
                  nullable=False),
    ...
    op.create_index('idx_pipeline_status', 'development_pipeline', ['development_status'])
    op.create_index('idx_pipeline_completion', 'development_pipeline', ['expected_completion'])
    op.create_index('idx_pipeline_location', 'development_pipeline', ['location'], postgresql_using='gist')
    ...
    op.create_table('development_analyses',
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        sa.Column('property_id', postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column('estimated_revenue', sa.DECIMAL(15, 2), nullable=True),
        sa.Column('projected_roi_percentage', sa.DECIMAL(5, 2), nullable=True),
    ...
    op.create_index('idx_analysis_property_date', 'development_analyses', ['property_id', 'analysis_date'])
    op.create_index('idx_analysis_type', 'development_analyses', ['analysis_type'])
    
    # Market Intelligence tables
    
    # Yield Benchmarks table
    op.create_table('yield_benchmarks',
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        ...
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('benchmark_date', 'property_type', 'district', name='uq_benchmark_date_type_location')
    )
    op.create_index('idx_benchmark_date', 'yield_benchmarks', ['benchmark_date'])
    op.create_index('idx_benchmark_location', 'yield_benchmarks', ['district', 'subzone'])
    
    # Absorption Tracking table
    op.create_table('absorption_tracking',
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
    ...
    op.create_index('idx_absorption_project_date', 'absorption_tracking', ['project_id', 'tracking_date'])
    op.create_index('idx_absorption_type_date', 'absorption_tracking', ['property_type', 'tracking_date'])
    
    # Market Cycles table
    op.create_table('market_cycles',
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        ...
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('cycle_date', 'property_type', 'market_segment', name='uq_cycle_date_type_segment')
    )
    ...
    op.create_table('market_indices',
        ...
        sa.Column('property_type', sa.Enum('office', 'retail', 'industrial', 'residential', 'mixed_use',
                                          'hotel', 'warehouse', 'land', 'special_purpose',
                                          name='property_type', create_type=False), nullable=True),
    ...
    op.create_table('competitive_sets',
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        ...
        sa.Column('property_type', sa.Enum('office', 'retail', 'industrial', 'residential', 'mixed_use',
                                          'hotel', 'warehouse', 'land', 'special_purpose',
                                          name='property_type', create_type=False), nullable=False),
        sa.Column('location_bounds', Geography(geometry_type='POLYGON', srid=4326), nullable=True),
