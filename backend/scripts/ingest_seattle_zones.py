"""Ingest Seattle zoning overlays into ``ref_zoning_layers``.

Fetches zoning polygons from the Seattle Open Data SODA endpoint or a local
...
Example usage (SODA fetch) ::
    ...
    PYTHONPATH=$REPO_ROOT \\
      .venv/bin/python -m backend.scripts.ingest_seattle_zones \\
      --dataset-id <SODA_ZONING_DATASET_ID> \\
      --app-token "$SEATTLE_SODA_APP_TOKEN" \\
      --layer-name seattle_zoning \\
      --persist

Defaults:
- Source CRS: EPSG:4326. Override with --source-epsg if the export is in StatePlane
  (e.g., EPSG:2926).
"""
...
SODA_DEFAULT_ZONING_ID = "n8h3-r7is"  # Seattle zoning (SODA alternative)
...
def _should_transform_to_wgs84(geojson: dict[str, Any]) -> Tuple[bool, str]:
    ...
    if declared_crs:
        ...
        if "2926" in normalized or "stateplane" in normalized:
...
def _normalise_zone_feature(feature: dict[str, Any]) -> ZoneRecord:
    ...
    zone_code = (
        properties.get("zone")  # common Seattle zoning field
        or properties.get("zoning")
        or properties.get("zoning_symbol")
    ...
    attributes = {
        ...
        "zoning_symbol": properties.get("zoning_symbol"),
        "zoning_description": properties.get("zoning_description"),
...
async def ingest_seattle_zones(options: ZoningIngestionOptions) -> dict[str, Any]:
    # Step 1: load geojson from file or SODA
    ...
    # Step 2: CRS check/transform
    needs_transform, reason = _should_transform_to_wgs84(geojson)
    if needs_transform:
        logger.info("seattle_zones:transform_auto", reason=reason)
        geojson = _transform_coordinates(geojson, options.source_epsg)
    else:
        logger.info("seattle_zones:transform_auto_skipped", reason=reason)
    ...
    if options.persist:
        ...
        for feature in geojson.get("features", []):
            try:
                ...
                logger.warning("seattle_zones:zone_skip", reason=str(exc))
        ...
        if zone_records:
            ...
            logger.info(
                "seattle_zones:ref_zoning_persisted",
                persisted_records=persisted,
                layer_name=options.layer_name,
            )
        else:
            logger.warning("seattle_zones:zone_records_empty")

    logger.info(
        "seattle_zones:completed",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Seattle zoning overlays into ref_zoning_layers."
    )
    parser.add_argument(
        "--input-path",
        type=Path,
        default=None,
        help="Path to zoning GeoJSON. If omitted, fetches from SODA.",
    )
    parser.add_argument(
        "--dataset-id",
        type=str,
        default=SODA_DEFAULT_ZONING_ID,
        help="SODA dataset ID for zoning polygons (e.g., Seattle zoning layer).",
    )
    parser.add_argument(
        "--app-token",
        type=str,
        default=None,
        help="Optional SODA app token (defaults to SEATTLE_SODA_APP_TOKEN).",
    )
    parser.add_argument(
        "--layer-name",
        type=str,
        default="seattle_zoning",
        help="Layer name stored in ref_zoning_layers (default: seattle_zoning).",
    )
    parser.add_argument(
        "--max-features",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="reset_layer",
        help="Do not delete existing SEA rows for the layer before inserting.",
    )
    parser.add_argument(
        "--source-epsg",
        type=int,
        default=4326,
        help="EPSG code of the source dataset (default: 4326 WGS84).",
    )
...
def main() -> None:
    ...
    result = asyncio.run(ingest_seattle_zones(options))

    print("\nSeattle Zoning Ingestion Result:")
    print("-" * 60)
