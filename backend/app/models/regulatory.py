...
class RegulatoryAgency(Base):
    ...
    id: Mapped[UUIDValue] = mapped_column(UUID(), primary_key=True, default=uuid4)
...
class AuthoritySubmission(Base):
    ...
    project_id: Mapped[UUIDValue] = mapped_column(
        UUID(), ForeignKey("projects.id"), nullable=False
    )
    agency_id: Mapped[UUIDValue] = mapped_column(
        UUID(), ForeignKey("regulatory_agencies.id"), nullable=False
    )
...
class SubmissionDocument(Base):
    ...
    id: Mapped[UUIDValue] = mapped_column(UUID(), primary_key=True, default=uuid4)
    submission_id: Mapped[UUIDValue] = mapped_column(
        UUID(), ForeignKey("authority_submissions.id"), nullable=False
    )
    ...
    uploaded_by_id: Mapped[Optional[UUIDValue]] = mapped_column(
