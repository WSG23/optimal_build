...
class ZohoClient:
    ...
    def _require_client(self) -> httpx.AsyncClient:
        if self._mock_mode:
            raise RuntimeError("Zoho HTTP client is unavailable in mock mode")
        if self.client is None:
            try:
                self.client = httpx.AsyncClient(timeout=30.0)
            except RuntimeError as exc:  # pragma: no cover
                logger.warning("httpx AsyncClient unavailable; Zoho disabled")
                raise RuntimeError("Zoho HTTP client is unavailable") from exc
        return self.client
    ...
    async def refresh_tokens(self, refresh_token: str) -> OAuthBundle:
        if self._mock_mode:
            ...
            return OAuthBundle(
                access_token=f"zoho-access-{suffix}",
                refresh_token=f"zoho-refresh-{suffix}",
                expires_at=utcnow() + timedelta(hours=1),
            )
        self._require_credentials()
        client = self._require_client()
    ...
    async def publish_listing(
        ...
        if self._mock_mode:
            listing_id = str(payload.get("external_id") or "zoho-mock-lead")
