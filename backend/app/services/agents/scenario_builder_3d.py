...
class Quick3DScenarioBuilder:
    ...
    async def generate_massing_scenarios(
        ...
        property_boundary = await self._get_property_boundary(
    ...
    async def _get_property_boundary(
        ...
        if property_data.location:
            ...
            result = await session.execute(
                select(ST_AsText(property_data.location))
            )
    ...
    async def _generate_new_build_massing(
        ...
        # Generate 3D mesh
        massing.mesh = self._create_building_mesh(
            buildable_footprint, max_height
        )
    ...
    async def _generate_mixed_use_massing(
        ...
        massing.gfa = self._calculate_mixed_use_gfa(
            buildable_footprint, tower_footprint,
    ...
    async def _generate_vertical_extension(
        ...
        massing.volume = self._calculate_volume(massing.mesh)
        massing.gfa = self._calculate_gfa(footprint, existing_floors) + \
    ...
    async def _generate_phased_development(
        ...
        phase1_footprint, phase2_footprint = self._split_footprint(
            buildable_footprint
        )
    ...
    def _get_default_use_mix(self, property_type: PropertyType) -> Dict[str, float]:
        ...
        use_mixes = {
            ...
            PropertyType.INDUSTRIAL: {"industrial": 0.7, "office": 0.2, "warehouse": 0.1},
    ...
    def _split_footprint(
        ...
        return (
            list(phase1.exterior.coords),
            list(phase2.exterior.coords)
        )
