...
class GPSPropertyLogger:
        ...
        try:
            ...
            if property_record:
                ...
                property_id = await self._create_property(
                    session, address, latitude, longitude, 
                    ura_zoning, existing_use, property_info
                )
    ...
    async def _find_existing_property(
        ...
        if address.postal_code:
            stmt = select(Property).where(
                Property.postal_code == address.postal_code
            )
            result = await session.execute(stmt)
            property_record = result.scalar_one_or_none()
    ...
    async def _create_property(
        ...
        property_data = {
            "id": property_id,
            "name": property_info.property_name if property_info else address.building_name or "Unknown Property",
        ...
        if property_info:
            property_data.update({
                "land_area_sqm": Decimal(str(property_info.site_area_sqm)) if property_info.site_area_sqm else None,
                "gross_floor_area_sqm": Decimal(str(property_info.gfa_approved)) if property_info.gfa_approved else None,
                "building_height_m": Decimal(str(property_info.building_height)) if property_info.building_height else None,
    ...
    async def _update_property(
        ...
        if property_info and property_info.gfa_approved:
            property_record.gross_floor_area_sqm = Decimal(str(property_info.gfa_approved))
    ...
    def _determine_property_type(self, existing_use: str) -> PropertyType:
        ...
        elif "industrial" in use_lower or "warehouse" in use_lower or "factory" in use_lower:
            return PropertyType.INDUSTRIAL
        elif "residential" in use_lower or "condo" in use_lower or "apartment" in use_lower:
