...
class InvestmentMemorandumGenerator(PDFGenerator):
    ...
    async def generate(
        ...
        market_data = await self._load_market_intelligence(property_data['property'], session)
        ...
        # 3. Executive Summary
        story.extend(self._create_executive_summary(property_data, financial_data, market_data))
        story.append(PageBreak())
    ...
    async def _load_property_data(self, property_id: UUID, session: AsyncSession) -> Dict[str, Any]:
        ...
        ).order_by(MarketTransaction.transaction_date.desc()).limit(5)
        result = await session.execute(stmt)
    ...
    async def _calculate_financials(self, property_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        estimated_value = float(property_obj.gross_floor_area_sqm or 0) * 10.764 * 3000  # PSF
        ...
        valuation = value_property_multiple_approaches(
            ...
            property_size_sqf=Decimal(str((property_obj.gross_floor_area_sqm or 0) * 10.764))
        )
    ...
    async def _load_market_intelligence(
        ...
        # Market cycle
        stmt = select(MarketCycle).where(
            MarketCycle.property_type == property_obj.property_type
        ).order_by(MarketCycle.cycle_date.desc()).limit(1)
        result = await session.execute(stmt)
    ...
    def _create_executive_summary(
        ...
        thesis_list = ListFlowable(
            [ListItem(Paragraph(p, self.styles['Normal']), leftIndent=20) for p in thesis_points],
            bulletType='bullet'
        )
    ...
    def _create_investment_highlights(
        ...
        # Visual highlights
        highlights = [
            {'label': 'Cap Rate', 'value': f"{float(metrics.cap_rate or 0) * 100:.2f}%"},
            {'label': 'NOI', 'value': f"${float(metrics.noi) / 1000000:.1f}M"},
            {'label': 'Occupancy', 'value': f"{(1 - property_data['vacancy_rate']) * 100:.0f}%"},
            {'label': 'GFA', 'value': f"{property_data['property'].gross_floor_area_sqm / 1000:.0f}k sqm"}
        ]
        ...
        # Value creation
        story.append(Paragraph("Value Creation Opportunities", self.styles['CustomHeading2']))
        ...
        value_list = ListFlowable(
            [ListItem(Paragraph(p, self.styles['Normal']), leftIndent=20) for p in value_points],
            bulletType='bullet'
        )
    ...
    def _create_property_overview(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        story.append(Paragraph("Physical Specifications", self.styles['CustomHeading2']))
        
        specs_data = [
            ["Specification", "Details"],
            ["Land Area", self.format_area(property_obj.land_area_sqm or 0)],
            ["Gross Floor Area", self.format_area(property_obj.gross_floor_area_sqm or 0)],
            ["Net Lettable Area", self.format_area(property_obj.net_lettable_area_sqm or 0)],
            ["Floors", f"{property_obj.floors_above_ground or 0} above ground"],
            ["Typical Floor Plate", self.format_area(
                (property_obj.net_lettable_area_sqm or 0) / (property_obj.floors_above_ground or 1)
            )],
        ...
        # Accessibility
        story.append(Paragraph("Accessibility & Transportation", self.styles['CustomHeading2']))
        ...
        transport_list = ListFlowable(
            [ListItem(Paragraph(p, self.styles['Normal']), leftIndent=20) for p in transport_points],
            bulletType='bullet'
        )
    ...
    def _create_location_analysis(
        ...
        # Micro location
        story.append(Paragraph("Micro Location Analysis", self.styles['CustomHeading2']))
    ...
    def _create_financial_analysis(self, financial_data: Dict[str, Any]) -> List[Any]:
        ...
        income_data = [
            ...
            ["Base Rent", self.format_currency(financial_data['annual_rental_income']), "95%"],
            ["Parking Income", self.format_currency(financial_data['annual_rental_income'] * 0.03), "3%"],
            ["Other Income", self.format_currency(financial_data['annual_rental_income'] * 0.02), "2%"],
            ["Gross Income", self.format_currency(financial_data['annual_rental_income'] * 1.05), "100%"],
            ["Less: Vacancy", f"({self.format_currency(financial_data['annual_rental_income'] * 0.05)})", "(5%)"],
            ["Effective Gross Income", self.format_currency(financial_data['annual_rental_income']), "95%"]
        ]
        ...
        expense_data = [
            ...
            ["Property Management", self.format_currency(financial_data['operating_expenses'] * 0.15), "$15"],
            ["Utilities", self.format_currency(financial_data['operating_expenses'] * 0.25), "$25"],
            ["Maintenance & Repairs", self.format_currency(financial_data['operating_expenses'] * 0.20), "$20"],
            ["Security", self.format_currency(financial_data['operating_expenses'] * 0.15), "$15"],
            ["Insurance", self.format_currency(financial_data['operating_expenses'] * 0.10), "$10"],
            ["Property Tax", self.format_currency(financial_data['operating_expenses'] * 0.15), "$15"],
            ["Total OpEx", self.format_currency(financial_data['operating_expenses']), "$100"]
        ]
    ...
    def _create_investment_returns(
        ...
        # Return projections
        story.append(Paragraph("5-Year Return Projections", self.styles['CustomHeading2']))
        ...
        # IRR analysis
        story.append(Paragraph("Internal Rate of Return Analysis", self.styles['CustomHeading2']))
        ...
        story.append(Paragraph("Return Sensitivity Analysis", self.styles['CustomHeading2']))
    ...
    def _create_risk_analysis(
        ...
        risk_matrix = [
            ...
            ["Tenant Default", "Low", "Medium", "Diversified tenant base, security deposits"],
            ...
            ["Environmental", "Low", "Medium", "Regular assessments, insurance coverage"]
        ]
        ...
        story.append(Paragraph("Tenant Concentration Risk", self.styles['CustomHeading2']))
    ...
    def _create_exit_strategies(
        ...
        # Exit valuation
        story.append(Paragraph("Exit Valuation Analysis", self.styles['CustomHeading2']))
    ...
    def _create_transaction_structure(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        terms_list = ListFlowable(
            [ListItem(Paragraph(t, self.styles['Normal']), leftIndent=20) for t in terms],
            bulletType='bullet'
        )
    ...
    def _create_appendices(self) -> List[Any]:
        ...
        assumptions_list = ListFlowable(
            [ListItem(Paragraph(a, self.styles['Normal']), leftIndent=20) for a in assumptions],
            bulletType='bullet'
        )
    ...
    def _calculate_vacancy_rate(self, rentals: List[RentalListing], property_obj: Property) -> float:
        """Calculate current vacancy rate."""
        if not property_obj.net_lettable_area_sqm:
            return 0.05  # Default 5%
        
        occupied_area = sum(
            rental.floor_area_sqm for rental in rentals 
