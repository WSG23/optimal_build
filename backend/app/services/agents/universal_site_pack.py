...
class UniversalSitePackGenerator(PDFGenerator):
    ...
    async def _load_property_data(
        ...
        stmt = (
            select(DevelopmentAnalysis)
            .where(DevelopmentAnalysis.property_id == property_id)
    ...
    async def _load_market_data(
        ...
        stmt = (
            ...
            .where(MarketTransaction.property_id == property_obj.id)
        ...
        return {"transactions": transactions, "benchmarks": benchmarks}
    ...
    def _create_executive_summary(
        ...
        # Key findings
        story.append(
            Paragraph("Key Investment Highlights", self.styles["CustomHeading2"])
        )
        ...
        highlight_list = ListFlowable(
            [
                ListItem(Paragraph(h, self.styles["Normal"]), leftIndent=20)
                for h in highlights
            ],
            bulletType="bullet",
        )
        ...
        if property_data["latest_analysis"]:
            ...
            story.append(
                Paragraph(
                    "Development Potential Summary", self.styles["CustomHeading2"]
                )
            )
            ...
            summary_data = [
                ["Parameter", "Value"],
                [
                    "Maximum GFA Potential",
                    self.format_area(analysis.gfa_potential_sqm or 0),
                ],
                ["Optimal Use Mix", self._format_use_mix(analysis.optimal_use_mix)],
                [
                    "Current Market Value",
                    self.format_currency(analysis.market_value_estimate or 0),
                ],
                ["Market Cap Rate", f"{analysis.projected_cap_rate or 0:.2f}%"],
                [
                    "Zoning Compliance",
                    "Compliant" if analysis.site_constraints else "Flexible",
                ],
                ["Development Timeline", "Subject to detailed planning"],
            ]

            table = self._create_data_table(
                summary_data, col_widths=[3 * inch, 3 * inch]
            )
            story.append(table)
            story.append(Spacer(1, 0.2 * inch))
    ...
    def _create_site_analysis(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        attributes_data = [
            ...
            [
                "Site Coverage",
                f"{((property_obj.gross_floor_area_sqm or 0) / (property_obj.land_area_sqm or 1)) * 100:.1f}%",
            ],
            ...
            [
                "Tenure",
                (
                    property_obj.tenure_type.value.replace("_", " ").title()
        ...
        table = self._create_data_table(
            attributes_data, col_widths=[2 * inch, 4 * inch]
        )
        story.append(table)
        story.append(Spacer(1, 0.2 * inch))
        ...
        # Accessibility
        story.append(
            Paragraph("Accessibility & Connectivity", self.styles["CustomHeading2"])
        )
    ...
    def _create_zoning_section(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        story.append(
            Paragraph("Planning Considerations", self.styles["CustomHeading2"])
        )
        ...
        story.append(
            Paragraph("Key Regulatory Requirements", self.styles["CustomHeading2"])
        )
        ...
        req_list = ListFlowable(
            [
                ListItem(Paragraph(r, self.styles["Normal"]), leftIndent=20)
    ...
    def _create_market_analysis(self, market_data: Dict[str, Any]) -> List[Any]:
        ...
        # Supply and demand
        story.append(
            Paragraph("Supply & Demand Dynamics", self.styles["CustomHeading2"])
        )
    ...
    def _create_development_scenarios(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        # Scenario overview
        story.append(
            Paragraph("Development Options Analysis", self.styles["CustomHeading2"])
        )
        ...
        story.append(
            Paragraph(
                "Scenario 1: Premium Grade A Office", self.styles["CustomHeading2"]
            )
        )
        ...
        table = self._create_data_table(
            office_data, col_widths=[2.5 * inch, 2.5 * inch]
        )
        story.append(table)
        story.append(Spacer(1, 0.2 * inch))
        ...
        story.append(
            Paragraph(
                "Scenario 2: Mixed-Use Development", self.styles["CustomHeading2"]
            )
        )
    ...
    def _create_financial_analysis(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        story.append(
            Paragraph("Indicative Cash Flow Projection", self.styles["CustomHeading2"])
        )
        ...
        # Market sensitivity
        story.append(
            Paragraph("Market Sensitivity Factors", self.styles["CustomHeading2"])
        )
    ...
    def _create_risk_assessment(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        # Risk mitigation
        story.append(
            Paragraph("Risk Mitigation Strategies", self.styles["CustomHeading2"])
        )
    ...
    def _create_implementation_timeline(
        ...
        timeline_data = [
            ...
            [
                "Regulatory Review",
                "4-6 months",
                "Planning guidelines, compliance check",
            ],
        ...
        milestone_list = ListFlowable(
            [
                ListItem(Paragraph(m, self.styles["Normal"]), leftIndent=20)
                for m in milestones
            ],
            bulletType="bullet",
        )
    ...
    def _create_appendix_disclaimers(self) -> List[Any]:
        ...
        # Contact information
        story.append(
            Paragraph("For Further Information", self.styles["CustomHeading2"])
        )
