...
class PhotoDocumentationManager:
        ...
        try:
            ...
            if 'latitude' in gps_data and 'longitude' in gps_data:
                ...
                lat = self._convert_gps_to_decimal(
                    gps_data['latitude'],
                    gps_data.get('lat_ref', 'N')
                )
                lon = self._convert_gps_to_decimal(
                    gps_data['longitude'],
                    gps_data.get('lon_ref', 'E')
                )
    ...
    def _extract_timestamp_from_exif(self, exif_data: Dict[str, Any]) -> Optional[datetime]:
    ...
    def _extract_camera_info_from_exif(self, exif_data: Dict[str, Any]) -> Dict[str, str]:
    ...
    async def _create_photo_record(
        ...
        if location:
            ...
            capture_location = WKTElement(point_wkt, srid=4326)
