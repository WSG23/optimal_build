...
class FloorPlanDiagram(Flowable):
    ...
    def __init__(self, floor_data: Dict[str, Any], width: float = 6*inch, height: float = 4*inch):
    ...
    def draw(self):
        ...
        self.canv.drawString(10, self.height - 20, f"Floor {self.floor_data.get('floor', 'Typical')}")
        ...
        if 'units' in self.floor_data:
            ...
            for i, unit in enumerate(self.floor_data['units']):
                ...
                self.canv.setFillColor(colors.HexColor('#e8f4fd') if unit['available'] else colors.HexColor('#f0f0f0'))
                self.canv.rect(x + 2, 22, unit_width - 4, plate_height - 4, fill=1, stroke=1)
...
class AmenityIcons(Flowable):
    ...
    def draw(self):
        ...
        for i, amenity in enumerate(self.amenities[:8]):  # Max 8 icons
            ...
            self.canv.circle(x + icon_size/2, y + icon_size/2, icon_size/2, fill=1, stroke=1)
            
            # Icon symbol (simplified)
            self.canv.setFont("Helvetica-Bold", 20)
            self.canv.setFillColor(colors.HexColor('#2c3e50'))
            symbol = self._get_icon_symbol(amenity)
            self.canv.drawCentredString(x + icon_size/2, y + icon_size/2 - 7, symbol)
...
class MarketingMaterialsGenerator(PDFGenerator):
    ...
    async def generate_email_flyer(
        ...
        c.setFillColor(colors.white)
        c.drawCentredString(width/2, height - 1*inch, property_data['property'].name)
        
        # Tagline
        c.setFont("Helvetica", 14)
        tagline = "Premium Space Available" if material_type == "lease" else "Exceptional Investment Opportunity"
        ...
        c.roundRect(0.5*inch, y_position - 2*inch, width - inch, 1.8*inch, 10, fill=1)
        ...
        c.roundRect(2*inch, y_position - 0.7*inch, width - 4*inch, 0.6*inch, 5, fill=1)
        
        c.setFont("Helvetica-Bold", 14)
        c.setFillColor(colors.white)
        cta_text = "Schedule a Viewing" if material_type == "lease" else "Request Investment Pack"
        c.drawCentredString(width/2, y_position - 0.5*inch, cta_text)
        
        # Footer contact
        c.setFont("Helvetica", 10)
        c.setFillColor(colors.HexColor('#7f8c8d'))
        c.drawCentredString(width/2, 0.5*inch, "Commercial Property Advisors | +65 6XXX XXXX | info@cpAdvisors.com")
        ...
        d = Drawing(45, 45, transform=[45./width_qr,0,0,45./height_qr,0,0])
        d.add(qr_code)
    ...
    def _create_property_highlights(
        ...
        highlights = InvestmentHighlight([
            {'label': 'Total GFA', 'value': f"{property_obj.gross_floor_area_sqm/1000:.0f}k sqm"},
            {'label': 'Available', 'value': f"{property_data['total_available']:,.0f} sqm"},
            {'label': 'Floor Plate', 'value': f"{(property_obj.gross_floor_area_sqm or 0)/(property_obj.floors_above_ground or 1)/1000:.1f}k sqm"},
        ])
        ...
        features_list = ListFlowable(
            [ListItem(Paragraph(f, self.styles['Normal']), leftIndent=20) for f in features],
            bulletType='bullet'
        )
    ...
    def _create_floor_plans(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        if property_data['rentals']:
            ...
            units_data = [["Floor", "Unit", "Size (sqm)", "Configuration", "Availability"]]
        ...
        # Typical floor plan
        story.append(Paragraph("Typical Floor Configuration", self.styles['CustomHeading2']))
        ...
        # Specifications
        story.append(Paragraph("Technical Specifications", self.styles['CustomHeading2']))
        ...
        specs_list = ListFlowable(
            [ListItem(Paragraph(s, self.styles['Normal']), leftIndent=20) for s in specs],
            bulletType='bullet'
        )
    ...
    def _create_amenities_section(self, property_data: Dict[str, Any]) -> List[Any]:
        ...
        # Amenity icons
        amenities = [
            "Parking", "24/7 Security", "High-Speed WiFi", "Meeting Rooms",
            "Cafeteria", "Gym Access", "Reception", "Elevators"
        ]
    ...
    def _create_photo_gallery(self, photos: List[PropertyPhoto]) -> List[Any]:
        ...
        story.append(Paragraph(
            "🎥 Virtual Tour Available - Contact us for access",
            virtual_tour_style
        ))
    ...
    def _create_availability_section(
        ...
        title = "AVAILABILITY & RENTAL RATES" if material_type == "lease" else "PRICING & TERMS"
        ...
        if material_type == "lease":
            ...
            # Lease terms
            story.append(Paragraph("Standard Lease Terms", self.styles['CustomHeading2']))
        ...
        story.append(self._add_disclaimer('leasing' if material_type == 'lease' else 'sales'))
    ...
    def _create_contact_section(
        ...
        steps = [
            "Contact our leasing team to discuss requirements" if material_type == "lease" 
        ...
        steps_list = ListFlowable(
            [ListItem(Paragraph(s, self.styles['Normal']), leftIndent=20, value=str(i+1)) 
        ...
        if not contact_info:
            contact_info = {
                ...
                "phone": "+65 6XXX XXXX",
                "email": "leasing@cpAdvisors.com" if material_type == "lease" else "sales@cpAdvisors.com",
        ...
        cta_text = (
            "SCHEDULE A VIEWING TODAY" if material_type == "lease" 
    ...
    def _get_ideal_tenants(self, property_type: PropertyType) -> List[str]:
        ...
        tenant_map = {
            ...
            PropertyType.RETAIL: [
                "F&B operators", "Fashion retailers",
                "Lifestyle brands", "Service providers"
            ],
            PropertyType.INDUSTRIAL: [
                "Logistics operators", "Light manufacturing",
                "R&D facilities", "Data centers"
            ],
            PropertyType.WAREHOUSE: [
                "3PL providers", "E-commerce fulfillment",
                "Distribution centers", "Cold storage operators"
            ]
        }
        
        return tenant_map.get(
            property_type, 
            ["Corporate tenants", "Service providers", "Retailers"]
        )