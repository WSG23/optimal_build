    ...
    async def fetch_transactions(
        ...
        for i in range(count):
            ...
            transactions.append({
                ...
                "buyer_type": random.choice(["Individual", "Company", "REIT", "Foreign"]),
...
class MarketDataService:
    ...
    async def sync_provider(
        ...
        for prop_type in property_types:
            try:
                ...
                stored = await self._store_rentals(
                    rentals, provider_name, session
                )
    ...
    async def _store_transactions(
        ...
        for txn_data in transactions:
            try:
                ...
                property_id = await self._get_or_create_property(
                    txn_data, session
                )
                ...
                transaction = {
                    ...
                    "transaction_date": datetime.fromisoformat(txn_data["transaction_date"]).date(),
    ...
    async def _store_rentals(
        ...
        for rental_data in rentals:
            try:
                ...
                rental = {
                    "property_id": property_id,
                    "listing_date": datetime.fromisoformat(rental_data["listing_date"]).date(),
                    ...
                    "asking_rent_monthly": Decimal(str(rental_data.get("asking_rent_monthly", 0))),
                    "asking_psf_monthly": Decimal(str(rental_data.get("asking_psf_monthly", 0))),
                    "floor_level": rental_data.get("floor_level"),
                    "available_date": datetime.fromisoformat(rental_data["available_date"]).date() if rental_data.get("available_date") else None,
    ...
    async def _store_indices(
        ...
        for index_data in indices:
            try:
                index_record = {
                    "index_date": datetime.fromisoformat(index_data["index_date"]).date(),
    ...
    async def _get_or_create_property(
        ...
        new_property = {
            ...
            "property_type": property_type_map.get(
                data.get("property_type", "office").lower(),
                PropertyType.OFFICE
            ),
            "location": point,
            "district": data.get("district"),
            "gross_floor_area_sqm": Decimal(str(data.get("floor_area_sqm", 0))) if data.get("floor_area_sqm") else None,
    ...
    async def _calculate_yield_benchmarks(
        self,
        session: AsyncSession
    ):
    ...
    async def _update_absorption_metrics(
        self,
        session: AsyncSession
    ):
