...
class MarketIntelligenceAnalytics:
    ...
    async def generate_market_report(
        ...
        recommendations = self._generate_recommendations(
            comparables, supply_dynamics, yield_benchmarks,
            absorption_trends, market_cycle
        )
    ...
    async def _analyze_yield_benchmarks(
        ...
        # Calculate trends
        cap_rate_trend = self._calculate_yield_trend(
            benchmarks, "cap_rate_median"
        )
        rental_trend = self._calculate_yield_trend(
            benchmarks, "rental_psf_median"
        )
    ...
    async def _analyze_absorption_trends(
        ...
        stmt = select(AbsorptionTracking).where(
            and_(
        ...
        if not absorption_data:
            return {
                "message": "No absorption data available",
                "metrics": {}
            }
        ...
        # Forecast
        absorption_forecast = self._forecast_absorption(
            absorption_data, months_ahead=6
        )
        
        return {
            "current_metrics": {
                "sales_absorption_rate": float(latest_data.sales_absorption_rate or 0),
                "leasing_absorption_rate": float(latest_data.leasing_absorption_rate or 0),
    ...
    async def _analyze_market_cycle(
        ...
        stmt = select(MarketCycle).where(
            MarketCycle.property_type == property_type
        ...
        cycle_position = {
            ...
            "indicators": {
                ...
                "transaction_volume_change": float(current_cycle.transaction_volume_change or 0),
    ...
    def _group_by_quarter(
        ...
        for txn in transactions:
            quarter = f"{txn.transaction_date.year}-Q{(txn.transaction_date.month-1)//3 + 1}"
    ...
    def _get_major_developments(
        ...
        sorted_projects = sorted(
            projects,
            key=lambda x: x.total_gfa_sqm or 0,
            reverse=True
        )[:limit]
        ...
        return [
            {
                ...
                "completion": proj.expected_completion.isoformat() if proj.expected_completion else None,
    ...
    def _calculate_yield_trend(
        self,
        benchmarks: List[YieldBenchmark],
        metric: str
    ) -> str:
    ...
    def _calculate_yoy_change(
        ...
        return {
            ...
            "transaction_volume_change_pct": float(
                ((current.total_transaction_value or 0) - (year_ago.total_transaction_value or 0)) /
    ...
    def _analyze_velocity_trend(
        self,
        absorption_data: List[AbsorptionTracking]
    ) -> str:
        ...
        recent_velocity = statistics.mean([
            d.avg_units_per_month or 0 
            for d in absorption_data[-3:]
        ])
    ...
    def _forecast_absorption(
        ...
        avg_absorption = statistics.mean([
            float(d.sales_absorption_rate or 0) 
            for d in absorption_data[-6:]
        ])
        
        # Simple linear projection
        current_absorbed = float(absorption_data[-1].sales_absorption_rate or 0)
        projected_absorbed = min(
            current_absorbed + (avg_absorption * months_ahead / 100),
            100
        )
    ...
    def _detect_seasonal_patterns(
        ...
        for data in absorption_data:
            ...
            monthly_absorption[month].append(
                float(data.sales_absorption_rate or 0)
            )
        ...
        if monthly_avg:
            ...
            return {
                ...
                "seasonality_strength": (peak_month[1] - low_month[1]) / low_month[1] if low_month[1] else 0
            }
    ...
    async def _get_market_indices(
        ...
        stmt = select(MarketIndex).where(
            MarketIndex.property_type == property_type
        ).order_by(MarketIndex.index_date.desc()).limit(12)
        
        result = await session.execute(stmt)
        return result.scalars().all()
    
    def _analyze_index_trends(
        self,
