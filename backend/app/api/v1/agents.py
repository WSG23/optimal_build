...
async def log_property_by_gps(
    ...
    try:
        ...
        fallback = _build_unavailable_gps_response(
            latitude=request.latitude,
            longitude=request.longitude,
            scenarios=request.development_scenarios,
            jurisdiction_code=request.jurisdiction_code,
...
def _build_unavailable_gps_response(
    *,
    latitude: float,
    longitude: float,
    ...
    """Return an explicit unavailable response when capture services fail."""
    ...
    address = Address(
        full_address=f"Unresolved address near ({latitude:.4f}, {longitude:.4f})",
        country="Singapore",
    )
