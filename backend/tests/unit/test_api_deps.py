...
class TestRequestIdentity:
    def test_from_headers_defaults_to_configured_role(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(deps.settings, "DEFAULT_ROLE", "Admin")
        identity = deps.RequestIdentity.from_headers(
    ...
    def test_from_headers_rejects_invalid_role(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
@pytest.mark.no_db
@pytest.mark.asyncio
async def test_get_identity_returns_request_identity(
...
@pytest.mark.no_db
@pytest.mark.asyncio
async def test_get_request_role_delegates_to_identity(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.no_db
@pytest.mark.asyncio
async def test_require_reviewer_allows_reviewer_role(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.no_db
@pytest.mark.asyncio
async def test_require_reviewer_respects_viewer_mutation_override(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
