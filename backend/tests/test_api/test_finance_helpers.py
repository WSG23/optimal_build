...
def test_normalise_project_id_accepts_str_and_int():
    ...
    assert finance_common.normalise_project_id(project_uuid.int) == UUID(
        int=project_uuid.int
    )

    with pytest.raises(HTTPException):
        finance_common.normalise_project_id("not-a-uuid")
...
async def test_ensure_project_owner_honours_admin(monkeypatch):
    ...
    await finance_scenarios._ensure_project_owner(session, uuid4(), identity)
...
def test_build_construction_interest_schedule_calculates_interest():
    ...
    loan_schema, metadata = finance_export.build_construction_interest_schedule(
        schedule,
        currency="SGD",
...
def test_decimal_conversion_and_json_helpers():
    assert finance_common.decimal_from_value("1.23") == Decimal("1.23")
    assert finance_common.json_safe({"value": Decimal("1.5")}) == {"value": "1.5"}
    assert finance_common.format_percentage_label(Decimal("0.123")) == "+0.12%"
...
def test_default_job_status_and_parse_datetime():
    job = finance_common.default_job_status(42)
    assert job.scenario_id == 42
    dt = datetime.utcnow()
    assert finance_common.parse_datetime_value(dt) == dt
    assert finance_common.parse_datetime_value(dt.isoformat()) == dt
    assert finance_common.parse_datetime_value("invalid") is None


def test_scenario_job_statuses_and_record_job():
    scenario = SimpleNamespace(id=1, assumptions={"async_jobs": {"sensitivity": []}})
    jobs = finance_common.scenario_job_statuses(scenario)
    assert jobs[0].status == "completed"

    new_job = finance_common.default_job_status(1).model_copy(
        update={"status": "queued"}
    )
    finance_common.record_async_job(scenario, new_job)
    recorded_jobs = finance_common.scenario_job_statuses(scenario)
    assert recorded_jobs[-1].status == "queued"

    payload = finance_common.status_payload(
...
def test_compute_scalar_and_convert_dscr_entry():
    base = finance_common.CostIndexSnapshot(
        period="2020",
        value=Decimal("2.0"),
        unit="index",
        source="Base",
    ...
    assert finance_common.compute_scalar(base, latest) == Decimal("1.1000")
    ...
    schema = finance_common.convert_dscr_entry(dscr_entry)
