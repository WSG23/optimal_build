        ...
        for source in self._sources:
            for file in sorted(source.rglob("*.py")):
                ...
                if not self._should_track(file):
                    continue
                executed = executed_by_file.get(file.resolve(), set())
                executable = _find_executable_lines(file)
                reports.append(_FileCoverage(path=file, executed=executed, executable=executable))
        return reports
...
def _find_executable_lines(file: Path) -> set[int]:
    ...
    for node in ast.walk(tree):
        ...
        if end_lineno is not None:
            executable.update(range(lineno, end_lineno + 1))

    return {
        line
        for line in executable
        if line <= len(source.splitlines()) and line not in docstring_lines
    }
