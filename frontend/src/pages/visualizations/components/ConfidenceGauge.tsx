...
export function ConfidenceGauge({ label, value, projection }: ConfidenceGaugeProps) {
  const theme = useTheme();

  // Color logic
  let color = theme.palette.error.main;
  if (value >= 70) color = theme.palette.success.main; // "Green/Teal"
  else if (value >= 50) color = theme.palette.warning.main; // "Yellow/Orange"

  return (
    <Tooltip
        title={<Typography variant="caption">Projection: {projection}</Typography>}
