import { Box, Typography, useTheme, alpha, Fade, Paper, IconButton } from '@mui/material';
...
export function CorrelationHeatmap({ data }: CorrelationHeatmapProps) {
    ...
    return (
        <Box display="flex" gap={2} sx={{ position: 'relative', minHeight: 400 }}>
            {/* Heatmap Grid Area */}
            <Box flex={1}>
                <Typography variant="subtitle2" gutterBottom sx={{color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '0.1em'}}>
                    ...
                    {data.map((item) => (
                        <Box
                            ...
                            sx={{
                                ...
                                border: selectedCorrelation?.id === item.id ? `2px solid ${theme.palette.primary.main}` : '1px solid transparent',
                                display: 'flex',
                                alignItems: 'center',
                        ...
                        >
                            <Typography variant="caption" fontWeight="bold" sx={{ color: '#fff', textShadow: '0 1px 2px rgba(0,0,0,0.8)' }}>
                ...
                <Fade in={!!selectedCorrelation}>
                    ...
                    >
                        <Box display="flex" justifyContent="space-between" mb={2}>
                            ...
                            <IconButton size="small" onClick={() => setSelectedCorrelation(null)}>
                        ...
                        <Box my={2} textAlign="center">
                            <Typography variant="h4" fontWeight={800} sx={{ color: getColor(selectedCorrelation.coefficient), filter: 'brightness(1.2)' }}>
                        ...
                        <Typography variant="caption" color="text.secondary">AFFECTS</Typography>
                        <Typography variant="body1" fontWeight={600} gutterBottom>
                            {selectedCorrelation.outcome}
                        </Typography>

                        <Typography variant="body2" sx={{ mt: 2, p: 1.5, bgcolor: alpha(theme.palette.action.hover, 0.05), borderRadius: 2 }}>
