  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto p-2">
      {Object.entries(ticketsByStatus).map(([status, tickets]) => (
        <div key={status}>
          <h3 className="text-xs uppercase tracking-wide text-white/50">{status.replace(/_/g, ' ')}</h3>
            ...
            {tickets.map((ticket) => {
              ...
              return (
                <li key={ticket.id}>
                  ...
                  >
                    <div className="font-medium text-white/90">{ticket.title}</div>
                    <div className="text-xs text-white/60">{ticket.description}</div>
