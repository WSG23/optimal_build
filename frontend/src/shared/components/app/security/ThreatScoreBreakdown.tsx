...
export const ThreatScoreBreakdown: FC<ThreatScoreBreakdownProps> = ({
  threatData,
  compact,
}) => {
  return (
    <div className={`flex h-full flex-col ${compact ? 'gap-4' : 'gap-6'}`}>
      ...
      <p className="text-sm text-white/60">
        Entity <span className="text-white/90">{threatData.entity_id}</span>{' '}
        currently trends above baseline threat potential.
      </p>
    </div>
  )
}
