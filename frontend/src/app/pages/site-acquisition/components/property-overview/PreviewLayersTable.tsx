...
/** Only virtualize when the layer count exceeds this threshold */
const VIRTUALIZE_THRESHOLD = 20

/** Height of a collapsed row in pixels */
const ROW_HEIGHT = 44

/** Height of the expanded accordion content in pixels */
...
interface VirtualizedRowProps {
  layers: PreviewLayerMetadata[]
  renderRow: (layer: PreviewLayerMetadata, index: number) => ReactElement
}
...
function VirtualizedRow(
  ...
  const { index, style, layers, renderRow } = props
  return (
    <table
      className="preview-layers-master-table__table preview-layers-master-table__table--virtual-row"
      style={{ ...style, width: '100%' }}
    >
      <tbody>{renderRow(layers[index], index)}</tbody>
    </table>
  )
}
...
export function PreviewLayersTable({
  ...
  const getLegendEntry = useCallback(
    (layerName: string): PreviewLegendEntry | undefined => {
  ...
  const shouldVirtualize = layers.length > VIRTUALIZE_THRESHOLD
  ...
  const getRowHeight = useCallback(
    (index: number) => {
      ...
      const hasAccordion = hasLegendEditor && legendEntry
      return isExpanded && hasAccordion
        ? ROW_HEIGHT + ACCORDION_HEIGHT
        : ROW_HEIGHT
    },
    [expandedLayerId, layers, hasLegendEditor, getLegendEntry],
  )

  /** Renders the content for a single layer row (shared between virtualized and non-virtualized) */
  const renderLayerRow = (layer: PreviewLayerMetadata, index: number) => {
      ...
      <Fragment key={layer.id}>
        ...
        >
          ...
          <td className="preview-layers-master-table__td preview-layers-master-table__td--visibility">
            ...
            >
              {isVisible ? (
                <Visibility sx={{ fontSize: 'var(--ob-font-size-lg)' }} />
              ) : (
                <VisibilityOff sx={{ fontSize: 'var(--ob-font-size-lg)' }} />
              )}
            </button>
          </td>
          ...
          <td className="preview-layers-master-table__td preview-layers-master-table__td--numeric">
            {layer.metrics.gfaSqm != null
              ? formatNumber(layer.metrics.gfaSqm, {
                  maximumFractionDigits: layer.metrics.gfaSqm >= 1000 ? 0 : 1,
                })
              : '—'}
          </td>
          ...
          <td className="preview-layers-master-table__td preview-layers-master-table__td--numeric">
            {layer.metrics.niaSqm != null
              ? formatNumber(layer.metrics.niaSqm, {
                  maximumFractionDigits: layer.metrics.niaSqm >= 1000 ? 0 : 1,
                })
              : '—'}
          </td>
          ...
          <td className="preview-layers-master-table__td preview-layers-master-table__td--actions">
            ...
            <button
              ...
              title={isFocused ? 'Currently focused' : 'Focus on this layer'}
            >
            ...
            {hasLegendEditor && legendEntry && (
              ...
              >
                ...
                {isExpanded ? (
                  <ExpandLess sx={{ fontSize: 'var(--ob-font-size-sm)' }} />
                ) : (
                  <ExpandMore sx={{ fontSize: 'var(--ob-font-size-sm)' }} />
                )}
              </button>
            )}
          </td>
                ...
                <tr>
                  <td colSpan={columnCount} style={{ padding: 0 }}>
                    <VirtualList
                      ...
                      style={{
                        height:
                          Math.min(layers.length, MAX_VISIBLE_ROWS) *
                          ROW_HEIGHT,
                      }}
                    />
                  </td>
