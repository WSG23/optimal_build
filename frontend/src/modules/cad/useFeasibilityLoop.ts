...
export function useFeasibilityLoop(
            ...
            try {
                ...
                if (summary) {
                    setMetrics({
                        ...
                        savingsPercent: Math.round((summary.estimated_achievable_gfa_sqm / summary.max_permissible_gfa_sqm) * 100),
                        reviewHoursSaved: 12,
                        paybackWeeks: 4
                    });
                }
    ...
    const sendUpdate = useCallback(() => {
        ...
        const totalArea = unitsRef.current.reduce((acc, unit) => acc + unit.areaSqm, 0);
        const payload = {
            project: {
